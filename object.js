...
export const object = (() => {
  class NatureObject {
    ...
    LoadModels_() {
      ...
      const modelsToLoad = [
        ...
        },
        {
          ...
          boundingType: 'sphere',
          boundingRadius: 3,
          orbit: { radius: 20, speed:0.05 , angle: Math.random() * Math.PI * 2, height: 10, orbitType: 'horizontal' },
        ...
        },
        {
          ...
          boundingType: 'sphere',
          boundingRadius: 15,
          orbit: { radius: 35, speed:0.05 , angle: Math.random() * Math.PI * 2, horizontalOffset: 0, orbitType: 'vertical' },
          ka: 0x020202,
          ks: 0x020202,
        ...
        },
        {
          ...
          boundingType: 'sphere',
          boundingRadius: 5,
          orbit: { radius: 25, speed:0.05 , angle: Math.random() * Math.PI * 2, height: 15, orbitType: 'horizontal' },
          ka: 0x11fce5, // Example ambient color
          ks: 0x11fce5, // Example specular color
          kd: 0x11fce5, // Example diffuse color
        },
        {
          type: 'obj',
          ...
          boundingType: 'sphere',
          boundingRadius: 5,
          orbit: { radius: 30, speed:0.5 , angle: Math.random() * Math.PI * 2, horizontalOffset: 5, orbitType: 'vertical' },
          ka: 0xfc1111,
          ks: 0xfc1111,
    ...
    OnModelLoaded_(model, modelInfo) {
      ...
      let collidableRef = null; // Declare collidableRef at the top

      // Handle collidable setup if applicable, BEFORE central/orbiting object logic
      if (modelInfo.collidable) {
        let bounds, helper;
        collidableRef = { // Assign to collidableRef
          model: model,
        ...
        if (modelInfo.boundingType === 'box' && modelInfo.boundingBoxSize) {
          ...
          collidableRef.bounds = bounds; // Assign to collidableRef
          collidableRef.size = modelInfo.boundingBoxSize; // Assign to collidableRef
        ...
        if (bounds) {
          this.collidables_.push(collidableRef); // Push collidableRef
          helper.visible = false;
          this.debugHelpers_.push({helper: helper, collidable: collidableRef}); // Use collidableRef
      ...
      if (modelInfo.isCentral) {
        ...
        this.models_.push({ model: model, orbit: modelInfo.orbit, collidableRef: collidableRef, previousPosition: new THREE.Vector3() }); // Use collidableRef and initialize previousPosition
      }
    ...
    Update(timeElapsed) {
      ...
      this.models_.forEach(item => {
        const { model, orbit, collidableRef } = item;
        if (orbit) {
          // Store current position as previousPosition before updating
          item.previousPosition.copy(model.position);

          // Ensure orbit radius is larger than central object to avoid overlap
          const effectiveRadius = Math.max(orbit.radius, centralRadius + 10); // Add some buffer

          orbit.angle += orbit.speed * timeElapsed;

          let x, y, z;
          if (orbit.orbitType === 'horizontal') {
            x = centralPosition.x + effectiveRadius * Math.cos(orbit.angle);
            z = centralPosition.z + effectiveRadius * Math.sin(orbit.angle);
            y = centralPosition.y + orbit.height;
          } else if (orbit.orbitType === 'vertical') {
            // Vertical orbit: X or Z is fixed (horizontalOffset), Y and other horizontal axis vary
            // Let's assume vertical orbit along Y-Z plane for now, X is fixed by horizontalOffset
            x = centralPosition.x + (orbit.horizontalOffset || 0); // Fixed X or Z offset
            y = centralPosition.y + effectiveRadius * Math.sin(orbit.angle);
            z = centralPosition.z + effectiveRadius * Math.cos(orbit.angle);
          } else {
            // Default to horizontal if orbitType is not specified or unknown
            x = centralPosition.x + effectiveRadius * Math.cos(orbit.angle);
            z = centralPosition.z + effectiveRadius * Math.sin(orbit.angle);
            y = centralPosition.y + orbit.height;
          }
          ...
          // Update the bounding sphere's center to the new model position
          if (collidableRef && collidableRef.type === 'sphere') {
            collidableRef.bounds.center.copy(model.position);
          }
        }
      });
